var state.infoInfo ={
   "parameters" : {
      "icpsr" : "integer, numeric code for state used by the\nInter-university Consortium for Political and Social Research",
      "state" : "character, name of state or Washington\n\tD.C.",
      "year" : "numeric or NA, year of statehood"
   },
   "title" : "information about the American states needed for U.S. Congress",
   "reference" : "The function readKH converts the integer ICPSR codes\ninto strings, via a table lookup in this data frame.  Another table\nlookup in state.abb provides the 2-letter abbreviation\ncommonly used in identifying American legislators, e.g.,\nKENNEDY, E (D-MA).",
   "format" : "icpsrinteger, numeric code for state used by the\nInter-university Consortium for Political and Social Research\n\nstatecharacter, name of state or Washington\n\tD.C.\n\nyearnumeric or NA, year of statehood",
   "description" : "Description\nNumeric codes and names of 50 states and the District of Columbia,\nrequired to parse Keith Poole\nand Howard Rosenthal's collections of U.S. Congressional roll calls.",
   "usage" : "data(state.info)"
}

var state.info = [
   [
      "icpsr",
      "state",
      "year"
   ],
   [
      "1",
      "Connecticut",
      "1788"
   ],
   [
      "2",
      "Maine",
      "1820"
   ],
   [
      "3",
      "Massachusetts",
      "1788"
   ],
   [
      "4",
      "New Hampshire",
      "1788"
   ],
   [
      "5",
      "Rhode Island",
      "1790"
   ],
   [
      "6",
      "Vermont",
      "1791"
   ],
   [
      "11",
      "Delaware",
      "1787"
   ],
   [
      "12",
      "New Jersey",
      "1787"
   ],
   [
      "13",
      "New York",
      "1788"
   ],
   [
      "14",
      "Pennsylvania",
      "1787"
   ],
   [
      "21",
      "Illinois",
      "1818"
   ],
   [
      "22",
      "Indiana",
      "1816"
   ],
   [
      "23",
      "Michigan",
      "1837"
   ],
   [
      "24",
      "Ohio",
      "1803"
   ],
   [
      "25",
      "Wisconsin",
      "1848"
   ],
   [
      "31",
      "Iowa",
      "1846"
   ],
   [
      "32",
      "Kansas",
      "1861"
   ],
   [
      "33",
      "Minnesota",
      "1858"
   ],
   [
      "34",
      "Missouri",
      "1821"
   ],
   [
      "35",
      "Nebraska",
      "1867"
   ],
   [
      "36",
      "North Dakota",
      "1889"
   ],
   [
      "37",
      "South Dakota",
      "1889"
   ],
   [
      "41",
      "Alabama",
      "1819"
   ],
   [
      "42",
      "Arkansas",
      "1836"
   ],
   [
      "43",
      "Florida",
      "1845"
   ],
   [
      "44",
      "Georgia",
      "1788"
   ],
   [
      "45",
      "Louisiana",
      "1812"
   ],
   [
      "46",
      "Mississippi",
      "1817"
   ],
   [
      "47",
      "North Carolina",
      "1789"
   ],
   [
      "48",
      "South Carolina",
      "1788"
   ],
   [
      "49",
      "Texas",
      "1845"
   ],
   [
      "40",
      "Virginia",
      "1788"
   ],
   [
      "51",
      "Kentucky",
      "1792"
   ],
   [
      "52",
      "Maryland",
      "1788"
   ],
   [
      "53",
      "Oklahoma",
      "1907"
   ],
   [
      "54",
      "Tennessee",
      "1796"
   ],
   [
      "55",
      "Washington D.C.",
      ""
   ],
   [
      "56",
      "West Virginia",
      "1863"
   ],
   [
      "61",
      "Arizona",
      "1912"
   ],
   [
      "62",
      "Colorado",
      "1876"
   ],
   [
      "63",
      "Idaho",
      "1890"
   ],
   [
      "64",
      "Montana",
      "1889"
   ],
   [
      "65",
      "Nevada",
      "1864"
   ],
   [
      "66",
      "New Mexico",
      "1912"
   ],
   [
      "67",
      "Utah",
      "1896"
   ],
   [
      "68",
      "Wyoming",
      "1890"
   ],
   [
      "71",
      "California",
      "1850"
   ],
   [
      "72",
      "Oregon",
      "1859"
   ],
   [
      "73",
      "Washington",
      "1889"
   ],
   [
      "81",
      "Alaska",
      "1959"
   ],
   [
      "82",
      "Hawaii",
      "1959"
   ]
]
