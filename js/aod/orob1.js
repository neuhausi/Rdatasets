var orob1Info ={
   "usage" : "data(orob1)",
   "format" : "A data frame with 16 observations on the following 3 variables.",
   "title" : "Germination Data",
   "description" : "Description\n[Data describing the germination] &ldquo;for seed Orobanche cernua cultivated in three dilutions of a bean \nroot extract. The mean proportions of the three sets are 0.142, 0.872 and 0.842, and the overall mean is 0.614.&rdquo;\n(Crowder, 1978, Table 1).",
   "parameters" : {
      "dilution" : "a factor with 3 levels: 1/1, 1/25 and 1/625"
   }
}

var orob1 = [
   [
      "dilution",
      "n",
      "y"
   ],
   [
      "1/1",
      "43",
      "2"
   ],
   [
      "1/1",
      "51",
      "9"
   ],
   [
      "1/1",
      "44",
      "5"
   ],
   [
      "1/1",
      "71",
      "16"
   ],
   [
      "1/1",
      "24",
      "2"
   ],
   [
      "1/1",
      "7",
      "0"
   ],
   [
      "1/25",
      "19",
      "17"
   ],
   [
      "1/25",
      "56",
      "43"
   ],
   [
      "1/25",
      "87",
      "79"
   ],
   [
      "1/25",
      "55",
      "50"
   ],
   [
      "1/25",
      "10",
      "9"
   ],
   [
      "1/625",
      "13",
      "11"
   ],
   [
      "1/625",
      "62",
      "47"
   ],
   [
      "1/625",
      "104",
      "90"
   ],
   [
      "1/625",
      "51",
      "46"
   ],
   [
      "1/625",
      "11",
      "9"
   ]
]
